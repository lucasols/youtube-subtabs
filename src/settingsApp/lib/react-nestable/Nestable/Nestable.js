...
class Nestable extends Component {
  ...
  onMouseMove = e => {
    ...
    if (!this.elCopyStyles) {
      ...
      this.elCopyStyles = {
        ...
        // transform: `translate3d(${offset.left
        //   - clientX
        //   - scroll.left}px, ${offset.top - clientY - scroll.top}px)`,
        ...transformProps,
      };
    } else {
      this.elCopyStyles = {
  ...
  render() {
    ...
    const { group, className, maxDepth, onClick } = this.props;
    const options = this.getItemOptions();

    return (
      <div
        className={cn(className, `nestable-${group}`, {
